...
export default function TemplateEditor({ templateId, onBack, token, onTemplateSaved }) {
  ...
  const handleSave = async () => {
    ...
    const handleStartTour = useCallback(() => {
      ...
      setShowMusicOptions(false); // Start with this collapsed, will open when needed
    ...
    // Only stop tour when explicitly finished or skipped
    if ([STATUS.FINISHED, STATUS.SKIPPED].includes(status)) {
      setRunTemplateTour(false);
      return;
    }
    
    // Handle missing targets by trying to open sections
  ...
  }, [setRunTemplateTour, setShowMusicOptions, setShowEpisodeStructure, setShowAiSection]);
    ...
    },
    {
      ...
      content: podcasts.length === 0
        ...
        : 'Give your template a memorable name (like "Weekly Episode" or "Interview Format"). It\'s already connected to your show, but you can change that if you create multiple shows later.',
    },
    {
      target: '[data-tour="template-schedule"]',
      title: 'Auto-Publish Schedule (Optional)',
      content: 'Want episodes to publish automatically on a schedule? After saving your template, you can set specific days and times. Each new episode will pick the next available slot. Skip this if you prefer publishing manually!',
    },
    {
      target: '[data-tour="template-ai-guidance"]',
      title: 'AI Content Guidance (Optional)',
      content: 'The AI can auto-generate episode titles, show notes, and tags based on your content. Here you can tell it your preferred style, tone, and keywords. The defaults work great, so this is totally optional!',
    },
    {
      target: '[data-tour="template-add"]',
      title: 'Add More Building Blocks',
      content: 'Your template already has intro, content, and outro segments. Want to add a commercial break or multiple intros? Use these buttons! You can always drag to reorder segments later.',
    },
    {
      target: '[data-tour="template-structure"]',
      title: 'Customize Each Segment',
      content: hasContentSegment
        ? 'Each segment can have its own audio clip or AI-generated voice. Edit scripts, upload clips, and adjust voices right here. The drag handles (⋮⋮) let you reorder segments in seconds. This is where you make your template unique!'
        : 'Drop in a Content segment first, then customize each part. You can upload audio clips, use AI voices, or mix both. Drag handles let you reorder everything easily.',
    },
    {
      target: '[data-tour="template-advanced"]',
      title: 'Advanced: Timing & Music',
      content: 'Want background music that fades in/out? Or segments that overlap smoothly? These advanced options give you fine control. Don\'t worry - the defaults work great, so only tweak these if you want to!',
    },
    {
      target: '[data-tour="template-save"]',
      title: 'Save Your Changes',
      content: 'Made some edits? Hit save! Every new episode you create will automatically use your template as a starting point. You can always come back and refine it as your show evolves.',
    },
