...
import { DistributionLogo } from "./distributionLogos";
        ...
        {!loading && !error ? (
          <div className="max-h-[65vh] space-y-4 overflow-y-auto pr-1">
            {items.map((item) => {
              ...
              const detailId = `distribution-details-${item.key}`;
              return (
                <div
                  key={item.key}
                  className={cn(
                    "rounded-lg border bg-card px-4 py-4 shadow-sm transition",
                    expanded
                ...
                >
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <button
                      ...
                      className="group flex w-full flex-1 items-start gap-3 text-left focus:outline-none"
                      aria-expanded={expanded}
                      aria-controls={detailId}
                    >
                      <DistributionLogo platformKey={item.key} name={item.name} className="mt-0.5" />
                    ...
                    <div className="flex flex-col items-stretch gap-2 md:w-auto md:items-end">
                  ...
                  {expanded ? (
                    <div id={detailId} className="mt-4 space-y-4">
                      ...
                      {item.disabled_reason ? (
                        <div className="rounded-md border border-yellow-500/50 bg-yellow-500/10 px-3 py-2 text-sm text-yellow-900 dark:text-yellow-100">
                          {item.disabled_reason}
                        </div>
                      ) : null}
