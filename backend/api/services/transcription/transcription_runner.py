...
def _assign_speakers_from_utterances(words: List[Dict[str, Any]], utterances: List[Dict[str, Any]]) -> None:
...
def run_assemblyai_job(audio_path: Path, cfg: RunnerCfg, log: List[str]) -> NormalizedResult:
    ...
    if use_webhook:
        ...
        webhook_payload = webhook_manager.wait_for_completion(transcript_id, wait_timeout)
        if webhook_payload is not None:
            status = webhook_payload.get("status")
            if status == "error":
                raise AssemblyAITranscriptionError(f"AssemblyAI error: {webhook_payload.get('error')}")
            if status == "completed":
                if not webhook_payload.get("words"):
                    webhook_payload = get_transcription(transcript_id, api_key=api_key, base_url=base_url, log=log, session=http)
                return _normalize_completed_transcript(cast(TranscriptResp, webhook_payload))
