...
def test_distribution_checklist_includes_defaults(authed_client: TestClient, podcast: Podcast):
    ...
    assert {"youtube_music", "podcast_index", "pocket_casts", "overcast", "castbox"}.issubset(keys)
    ...
    spotify = _get_item(items, "spotify")
    assert spotify["action_url"].startswith("https://podcasters.spotify.com/pod/dashboard/submit")

    tunein = _get_item(items, "tunein")
    assert tunein["action_url"].startswith("https://podcasters.tunein.com/submit")

    podchaser = _get_item(items, "podchaser")
    assert podchaser["action_url"] == "https://www.podchaser.com/podcasters"
