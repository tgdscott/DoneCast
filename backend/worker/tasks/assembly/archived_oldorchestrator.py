...
def _finalize_episode(
    *,
    ...
    # REQUIRED: Calculate duration/size BEFORE charging credits
    ...
    try:
        ...
        if output_filename:
            ...
            if not final_txt.exists():
                final_txt = TRANSCRIPTS_DIR / f"{sanitized_stem}.final.txt"
            if final_txt.exists():
            ...
            if not published_txt.exists():
                published_txt = TRANSCRIPTS_DIR / f"{sanitized_stem}.txt"
            if published_txt.exists():
                final_transcript_files.append(("published", published_txt))

            # ALSO upload JSON transcripts - critical for re-assembly and AI tools after rebuilds
            # These contain the word-level timings essential for the "Edit" feature
            json_candidates = [
            ...
            # Use a set to avoid duplicates if specific names match
            seen_files = set([str(p[1].resolve()) for p in final_transcript_files if p[1].exists()])
            
            for key, path in json_candidates:
                if path.exists():
            ...
            if transcript_urls:
                try:
                    ...
                    meta["transcripts"].update({
                        ...
                        "original_json_url": transcript_urls.get("original_json"),
                        "words_json_url": transcript_urls.get("words_json"),
                        "json_url": transcript_urls.get("json"),
                    })
                    episode.meta_json = json.dumps(meta)
                    logging.info(
    ...
    *,
    ...
    with session_scope() as session:
        ...
        try:
            ...
            # PATCH: Ensure cover_image_path is set from episode_details if missing in context
            # This fixes the missing cover art issue when running on legacy orchestrator
            if not getattr(media_context, 'cover_image_path', None) and episode_details.get("cover_image_path"):
                try:
                    media_context.cover_image_path = episode_details.get("cover_image_path")
                    logging.info(f"[assemble] PATCH: Manually injected cover_image_path from details: {media_context.cover_image_path}")
                except Exception as patch_err:
                    logging.warning(f"[assemble] PATCH: Failed to inject cover_image_path: {patch_err}")
